...
from models.semparser_diff_layer import SemparserRINE
from models.model_config import RobertaQueryNerConfig
from utils.utils import get_parser, set_random_seed, invert_mask
...
class BertLabeling(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        t_total = self.t_total if self.t_total is not None else (len(self.train_dataloader()) // (self.args.accumulate_grad_batches * num_gpus) + 1) * self.args.max_epochs
    ...
    def training_step(self, batch, batch_idx):
        ...
        tokens, token_type_ids, start_labels, end_labels, start_label_mask, end_label_mask, label_types = batch
        ...
        _, total_loss = self.model(tokens, token_type_ids, attention_mask, start_label_mask, \
                                                            end_label_mask, start_labels, end_labels, label_types)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        intent_mask = torch.zeros(self.num_label_types).to("cuda:0")
        num_intent = 23
        intent_mask[1:num_intent] = 1
        # print("----------------------------")
        while (cur_step < self.max_pred_step) and (cur_label != "[EOP]"):
            ...
            if cur_step == 1:
                entity_logit = entity_logit * intent_mask
            ...
            end_token = "Ġ]"
            start_token = f"Ġ[{self.id2label[entity_pred]}" if tmp_start_pred > 0 else f"[{self.id2label[entity_pred]}Ġ"
    ...
    def write_test_file(self, sentences):
        pseudo_labels = []
        for i_s, sent in enumerate(sentences):
            sent = re.sub(r" {2,}", " ", sent.strip()).strip()
            ...
            pseudo_label = {
                ...
                "qas_id": i_s,
                "org_label": f"[IN:GET_EVENT {sent} ]"
            }
            pseudo_labels.append(pseudo_label)
            
        json_test_path = os.path.join(self.data_dir, f"mrc-ner.test")
        json.dump(pseudo_labels, open(json_test_path, 'wt'), indent=1)
        
    def read_test_outputs(self): 
        json_test_path = os.path.join(self.args.default_root_dir, "test_logs", f"test_log.json")  
        return json.load(open(json_test_path))
