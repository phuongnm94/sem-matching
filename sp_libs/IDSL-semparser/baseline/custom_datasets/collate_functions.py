...
def collate_to_max_length_w_gnn(batch: List[List[torch.Tensor]], ignore_index=-100, pad_id=0) -> List[torch.Tensor]:
    """
    pad to maximum length of this batch
    Args:
        batch: a batch of samples, each contains a list of field data(Tensor):
    ...
    output.append(torch.stack([x[6] for x in batch]))

    max_length = max(x[7].shape[0] for x in batch)
    pad_output = torch.full([batch_size, max_length], pad_id, dtype=batch[0][0].dtype)
    for sample_idx in range(batch_size):
